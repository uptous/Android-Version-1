 ...
 * Created by Prakash .
 */
public class VolunteeredGeneralAdapter extends RecyclerView.Adapter<VolunteeredGeneralAdapter.VersionViewHolder> {

    List<SignUpDetailResponseModel.ItemsBean.VolunteersBean> listEntities;
    Activity activity;
    String mStringType;

    boolean isPotluck = false;

    public VolunteeredGeneralAdapter(Activity a, List<SignUpDetailResponseModel.ItemsBean.VolunteersBean> listEntities) {

        this.listEntities = listEntities;
        this.activity = a;
        mStringType = Prefs.getSignUpType(a);
        if (mStringType != null && mStringType.equalsIgnoreCase("Potluck/Party"))
            isPotluck = true;
    }
    ...
    public void onBindViewHolder(final VersionViewHolder versionViewHolder, int i) {
        ...
        if (listEntities.get(i).getAttendees() != 0 && isPotluck) {
